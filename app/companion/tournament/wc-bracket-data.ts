...
import { winnerCodeOf } from "../../lib/wc-champion";
...
/** The user's followed country codes, uppercased — the highlight set
...
export function followedCountrySet(
  ...
  for (const f of follows) {
    if (f.kind === "country") out.add(f.id.toUpperCase());
  }
  return out;
}
...
export function buildWCBracket(
  ...
  const resolveSlot = (slot: BracketSlot): BracketSlot => {
    ...
    // The winner is trusted data (winnerCodeOf never guesses); when the
    // feeder isn't decided yet it stays a pairing.
    const winner = winnerCodeOf(feeder);
