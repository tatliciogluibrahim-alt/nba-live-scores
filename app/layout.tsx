  ...
  return (
    ...
    >
      <head>
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `(function(){function s(m){try{var d=document.createElement('div');d.id='nns-fatal';d.style.cssText='position:fixed;inset:0;background:#f1ead8;color:#1a1612;padding:24px;padding-top:80px;font:14px ui-monospace,Menlo,monospace;z-index:99999;white-space:pre-wrap;overflow:auto;-webkit-overflow-scrolling:touch';d.textContent='No Noise — startup error\\n\\n'+m+'\\n\\nForce-close the app and reopen. If it persists, screenshot this screen.';document.body.appendChild(d)}catch(e){}}function benign(r){if(!r)return false;var n=(r&&r.name)||'';var m=(r&&r.message)||(typeof r==='string'?r:'')||'';if(n==='AbortError')return true;if(/Transition was aborted/i.test(m))return true;if(/invalid state/i.test(m)&&/transition/i.test(m))return true;return false}window.addEventListener('error',function(e){s((e.message||'unknown')+'\\nat '+(e.filename||'?')+':'+(e.lineno||'?'))});window.addEventListener('unhandledrejection',function(e){var r=e.reason;if(benign(r)){if(e.preventDefault)e.preventDefault();return}s('unhandled rejection: '+(r&&r.message||r||'unknown'))});})();`,
          }}
        />
      </head>
      <body>
